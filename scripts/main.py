...
def part1():
    ...
    results, curves, best_fit = run_randomized_optimizer('one-max', one_max_problem, results, seed)
    ...
    results, curves, best_fit = run_randomized_optimizer('four-peaks', four_peaks_problem, results, seed)
    ...
    results, curves, best_fit = run_randomized_optimizer('flip-flop', flip_flop_problem, results, seed)
...
def part2():
    ...
    wine = load_wine_data()
    # wine_train_x, wine_train_y, wine_test_x, wine_test_y = split_data_set(fetus, seed)
    wine_train_x, wine_train_y, wine_test_x, wine_test_y = split_data_set(wine, seed)

    # Random Hill Climbing
    nn_rhc = mlrose.NeuralNetwork(hidden_nodes=[10], curve=True, random_state=seed,
                                  clip_max=5,
    ...
    # Simulated Annealing
    nn_sa = mlrose.NeuralNetwork(hidden_nodes=[10], algorithm='simulated_annealing', curve=True, random_state=seed,
                                 clip_max=5,
                                 learning_rate=0.0001, early_stopping=True,
                                 max_iters=10000, max_attempts=100, pop_size=10)
    ...
    # Genetic Algorithm
    nn_ga = mlrose.NeuralNetwork(hidden_nodes=[10], algorithm='genetic_alg', curve=True, random_state=seed,
                                 clip_max=5,
                                 learning_rate=0.0001, early_stopping=True,
                                 max_iters=10000, max_attempts=50, pop_size=10)
